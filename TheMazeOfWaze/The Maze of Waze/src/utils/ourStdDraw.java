 ...
 *  @author Kevin Wayne
 */
public final class ourStdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	...
	private static ourStdDraw std = new ourStdDraw();
	...
	private static void init() {
		...
		draw.addMouseMotionListener(handler);

		draw.addMouseListener(handler);
	...
	private static JMenuBar createMenuBar() {
		...
		JMenuItem menuItem4 = new JMenuItem(" Custume   ");
		...
		menuItem5.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E,
				Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		Filemenu.add(menuItem5);
		
		JMenu Functionsmenu = new JMenu("Functions");
		menuBar.add(Functionsmenu);
		JMenuItem menuItem6 = new JMenuItem(" Is connected   ");
		menuItem6.addActionListener(std);
		menuItem6.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_I,
				Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		Functionsmenu.add(menuItem6);
		
		JMenuItem menuItem7 = new JMenuItem(" Connect Points   ");
		menuItem7.addActionListener(std);
		menuItem7.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_D,
				Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		Functionsmenu.add(menuItem7);
		
		JMenuItem menuItem8 = new JMenuItem(" Shortest way   ");
		menuItem8.addActionListener(std);
		menuItem8.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P,
				Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		Functionsmenu.add(menuItem8);		
		
		JMenuItem menuItem9 = new JMenuItem(" Tsp   ");
		menuItem9.addActionListener(std);
		menuItem9.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_T,
				Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		Functionsmenu.add(menuItem9);
	...
	private static Image getImage(String filename) {
		...
		if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
			URL url = ourStdDraw.class.getResource(filename);
			if (url != null)
				icon = new ImageIcon(url);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		{
			FileDialog chooser = new FileDialog(ourStdDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
			chooser.setVisible(true);
			String filename = chooser.getFile();
			if (filename != null) {
				ourStdDraw.save(chooser.getDirectory() + File.separator + chooser.getFile());
			}
		}
		...
		if (str.equals(" open   "))
		{
			FileDialog chooser = new FileDialog(ourStdDraw.frame, "Use a .png or .jpg extension", FileDialog.LOAD);
			chooser.setVisible(true);
			String filename = chooser.getFile();
			if (filename != null) {
				ourStdDraw.open(chooser.getDirectory() + File.separator + chooser.getFile());
		}

		if (str.equals(" New   "))
		{
			System.out.println("new");
			//GuiwithStdDraw.reset();
		}
		
		if (str.equals(" Custume   "))
		{
		...
		if (str.equals(" Connect Points   "))
		{
			
			System.out.println("chose to points on the screen");
			//GuiwithStdDraw.reset();
		}
		
		if (str.equals(" Shortest way   "))
		{
			//GuiwithStdDraw.reset();
	...
	//trying to do a open file dialog
	...
	/*public void actionPerformed2(ActionEvent e2) {
		...
		String filename = chooser.getFile();
		if (filename != null) {
			StdDraw.open(chooser.getDirectory() + File.separator + chooser.getFile());
		}
	}*/
	...
	public void mousePressed(MouseEvent e) {
		synchronized (mouseLock) {
			mouseX = ourStdDraw.userX(e.getX());
			mouseY = ourStdDraw.userY(e.getY());
			isMousePressed = true;
		}
	}
